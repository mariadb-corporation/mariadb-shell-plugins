...
def format_buckets_listing(buckets, current_bucket=None):
    ...
    for b in buckets:
        ...
        name = re.sub(
            r"[\n\r]", " ", b.name[:22] + ".." if len(b.name) > 24 else b.name
        )
...
def format_bucket_objects_listing(bucket_list_objects):
    ...
    for o in bucket_list_objects.objects:
        # Shorten to 24 chars max, remove linebreaks
        name = re.sub(
            r"[\n\r]", " ", o.name[:63] + ".." if len(o.name) > 65 else o.name
        )
        size = sizeof_fmt(o.size)
        time = (
            f"{o.time_modified:%Y-%m-%d %H:%M}" if o.time_modified is not None else ""
        )
...
def list_buckets(**kwargs):
    ...
    try:
        ...
        bucket_name = configuration.get_current_bucket_name(config=config)
        ...
        if return_formatted:
            return format_buckets_listing(buckets=buckets, current_bucket=bucket_name)
        else:
...
def get_bucket(
    ...
    try:
        ...
        if bucket_name is not None:
            try:
                ...
                if not interactive:
                    raise
                if e.status == 404:
                    print(f"The bucket with the name {bucket_name} was not " f"found.")
                else:
                    print(
                        f"ERROR: {e.message}. (Code: {e.code}; " f"Status: {e.status})"
                    )
                return
        ...
        bucket = core.prompt_for_list_item(
            item_list=buckets,
            prompt_caption=("Please enter the name or index " "of the bucket: "),
    ...
    try:
        ...
        if not interactive and bucket_name is None:
            ...
            bucket_name = mysqlsh.globals.shell.prompt(
                "Please enter the name for the new bucket: ", {"defaultValue": ""}
            ).strip()
            if bucket_name == "":
        ...
        create_bucket_details = oci.object_storage.models.CreateBucketDetails(
            name=bucket_name, compartment_id=compartment_id
        )

        bucket = os_client.create_bucket(
            namespace_name=namespace_name, create_bucket_details=create_bucket_details
        ).data
...
def delete_bucket(bucket_name=None, compartment_id=None, config=None, interactive=True):
    ...
    try:
        ...
        if obj_count > 0:
            print(
                f"The bucket {bucket.name} contains {obj_count} "
        ...
        for p in pars:
            os_client.delete_preauthenticated_request(
                namespace_name=namespace_name, bucket_name=bucket.name, par_id=p.id
            )

        os_client.delete_bucket(namespace_name=namespace_name, bucket_name=bucket.name)
...
def delete_file_from_list_from_bucket(
    index, os_client, objects, namespace_name, bucket_name, thread_count
):
        ...
        if name and (name == "*" or "*" in name):
            ...
            if name != "*":
                ...
                if "*" in name:
                    ...
                    objects = [obj for obj in objects if name == obj.name.lower()]
            ...
            if interactive:
                ...
                # Delete all objects
                print(f"Deleting {obj_count} " f"object{'s' if obj_count > 1 else ''}.")
            ...
            if interactive:
                print(
                    f"Bucket object{'s' if '*' in name else ''} "
                    f"deleted successfully."
                )
        elif name:
            os_client.delete_object(
                namespace_name=namespace_name, bucket_name=bucket.name, object_name=name
            )
...
def create_bucket_object(
    ...
    if file_name is None and file_content is None:
        file_name = mysqlsh.globals.shell.prompt(
            "Please enter the path to a local file to upload: ", {"defaultValue": ""}
        ).strip()
        if file_name == "":
            print("Operation cancelled.")
            return
...
def bucket_objects_upload_progress_callback(
    file_path, bytes_uploaded, total_file_size, send_gui_message
):
    ...
    try:
        ...
        def report_error(e, file_path):
            if isinstance(e, oci.exceptions.ServiceError):
                ...
                if send_gui_message is not None:
                    send_gui_message(
                        "data", {"filePath": file_path, "error": f"ERROR: {e}"}
                    )
                elif interactive:
                    print(f"{file_path} - ERROR: {e}")
        ...
        def progress_callback(data):
            if data["status"] == "ERROR":
                ...
                bucket_objects_upload_progress_callback(
                    data["file_path"],
                    data["bytes_uploaded"],
                    data["file_size"],
                    send_gui_message,
...
def list_bucket_objects(**kwargs):
    ...
    try:
        ...
        if name and name != "*":
            ...
            if "*" in name:
                name_pattern = "^" + name.replace("*", ".+")
                bucket_list_objects.objects = [
                    obj
                    for obj in bucket_list_objects.objects
                    if re.search(name_pattern, obj.name.lower())
                ]
            else:
                bucket_list_objects.objects = [
        ...
        if (
            len(bucket_list_objects.prefixes) + len(bucket_list_objects.objects) < 1
            and interactive
        ):
            if name:
                print(
                    f"The bucket {bucket.name} contains no objects matching "
        ...
        if return_formatted:
            return format_bucket_objects_listing(
                bucket_list_objects=bucket_list_objects
            )
        else:
            # return compartments in JSON text output
...
def upload_file_from_list_to_bucket(
    index, upload_manager, files, namespace_name, bucket_name, object_name_prefix
):
...
def create_bucket_objects_from_local_dir(
    local_dir_path=None,
    bucket_name=None,
    ...
    local_dir_path = os.path.abspath(os.path.expanduser(local_dir_path))
    if not os.path.exists(local_dir_path):
        print(f"Cannot find the file directory {local_dir_path}.")
        return

    file_list = [
        os.path.join(local_dir_path, name)
    ...
    try:
        ...
        ths = [
            threading.Thread(
                target=upload_file_from_list_to_bucket,
                args=(
                    i,
                    upload_manager,
                    file_list,
    ...
    try:
        ...
        if bucket_object_name is None:
            bucket_object_name = mysqlsh.globals.shell.prompt(
                "Please enter the name of the bucket object: ", {"defaultValue": ""}
            ).strip()
        ...
        if valid_till is None:
            ...
            try:
                valid_till = datetime.datetime(
                    *(time.strptime(valid_till, f"%Y-%m-%d %H:%M:%S")[0:6])
                )
            except ValueError as e:
        ...
        if access_type == "w":
            access = (
                oci.object_storage.models.CreatePreauthenticatedRequestDetails.ACCESS_TYPE_OBJECT_WRITE
            )
        elif access_type == "rw":
            access = (
                oci.object_storage.models.CreatePreauthenticatedRequestDetails.ACCESS_TYPE_OBJECT_READ_WRITE
            )
        else:
            access = (
                oci.object_storage.models.CreatePreauthenticatedRequestDetails.ACCESS_TYPE_OBJECT_READ
            )
...
def format_pars_listing(pars):
    ...
    for p in pars:
        ...
        name = re.sub(
            r"[\n\r]", " ", p.name[:28] + ".." if len(p.name) > 30 else p.name
        )
        obj_name = re.sub(
            r"[\n\r]",
            " ",
            p.object_name[:28] + ".." if len(p.object_name) > 30 else p.object_name,
        )
        time_cr = (
            f"{p.time_created:%Y-%m-%d %H:%M}" if p.time_created is not None else ""
        )
        time_ex = (
            f"{p.time_expires:%Y-%m-%d %H:%M}" if p.time_expires is not None else ""
        )
    ...
    try:
        ...
        if name:
            ...
            if "*" in name:
                name_pattern = "^" + name.replace("*", ".+")
                pars = [
                    par for par in pars if re.search(name_pattern, par.name.lower())
                ]
            else:
    ...
    try:
        ...
        if par_id is not None:
            par = os_client.get_preauthenticated_request(
                namespace_name=namespace_name, bucket_name=bucket_name, par_id=par_id
            ).data
            return par
        ...
        par = core.prompt_for_list_item(
            item_list=pars,
            prompt_caption=(
                "Please enter the index " "of the preauthenticated request: "
            ),
        )
    ...
    try:
        ...
        if not name:
            ...
            if par is None:
                ...
                return
            os_client.delete_preauthenticated_request(
                namespace_name=namespace_name, bucket_name=bucket.name, par_id=par.id
            ).data
        else:
            pars = oci.pagination.list_call_get_all_results(
                os_client.list_preauthenticated_requests,
