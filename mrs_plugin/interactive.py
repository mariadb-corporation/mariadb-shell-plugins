        ...
        if isinstance(schema_query, bytes):
            ...
            if service is not None:
                schema = lib.schemas.get_schema(
                    session, service_id=service["id"], request_path=f"/{request_path}"
                )
...
def resolve_db_object(
    ...
    if lib.core.get_interactive_default():
        schema = resolve_schema(
            session, schema_query=schema_id, service_query=service_id
        )
...
def resolve_content_file(
    ...
    if lib.core.get_interactive_default():
        ...
        content_files = lib.content_files.get_content_files(
            session, service_id=content_set["id"]
        )
...
def resolve_role(session, role_query: str | bytes):
    ...
    if len(role_query_list) == 3:
        ...
        service = lib.services.get_service(
            session, url_host_name=url_host_name, url_context_root=url_context_root
        )
        service_id = service["id"]
...
def resolve_auth_app(
    session,
    auth_app_query: str | bytes = None,
    service_query: str | bytes = None,
    required: bool = True,
):
...
def resolve_overwrite_file(file_path: str, overwrite: bool) -> None:
    ...
    if os.path.isdir(file_path):
        raise Exception(
            "Cancelling operation. Path already exists and it's a directory."
        )

    if overwrite is None and lib.core.get_interactive_default():
        overwrite = (
            lib.core.prompt(f"Overwrite {file_path}? [y/N]: ", {"defaultValue": "n"})
            .strip()
            .lower()
            == "y"
        )
