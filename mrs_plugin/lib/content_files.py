...
def format_content_file_listing(content_files, print_header=True):
    ...
    if print_header:
        output = (
            f"{'ID':>3} {'PATH':65} {'ENABLED':7} {'AUTH':4} "
    ...
    for item in content_files:
        i += 1
        path = (
            item["host_ctx"] + item["content_set_request_path"] + item["request_path"]
        )
...
def get_content_file(
    session,
    content_file_id: bytes | None = None,
        ...
        for file in sorted(files):
            ...
            options = {
                "last_modification": datetime.datetime.fromtimestamp(
                    pathlib.Path(fullname).stat().st_mtime, tz=datetime.timezone.utc
                ).strftime("%F %T.%f")[:-3],
            }
...
def add_content_file(
    session, content_set_id, request_path, requires_auth, data, enabled=1, options=None
):
...
def get_content_file_create_statement(session, content_file: dict) -> str:
    if "content" not in content_file:
        content_file = get_content_file(
            session, content_file_id=content_file["id"], include_file_content=True
        )

    output = []
    output.append(
        f"CREATE OR REPLACE REST CONTENT FILE {core.quote_rpath(content_file.get('request_path'))}"
    )
    output.append(
        f"    ON SERVICE {core.quote_rpath(content_file.get('host_ctx'))} CONTENT SET {core.quote_rpath(content_file.get('content_set_request_path'))}"
    )
    ...
    output.append(core.format_json_entry("OPTIONS", content_file.get("options")))

    auth = (
        "    AUTHENTICATION REQUIRED"
        if content_file["requires_auth"] in [True, 1]
