...
def get_status(session):
    ...
    result = {
        ...
        "service_upgrade_ignored": False,
        "service_count": 0,
        "service_being_upgraded": current_version[0] == 0
        and current_version[1] == 0
        and current_version[2] == 0,
    ...
    row = (
        lib.core.select(
            ...
            cols=[
                "service_enabled",
                "JSON_VALUE(data, '$.ignore_service_upgrades_till') as ignore_till",
            ],
            where="id=1",
        )
        .exec(session)
    ...
    if ignore_till != None:
        result["service_upgrade_ignored"] = [
            int(i) for i in ignore_till.split(".")
        ] >= current_version

    # Get the number of enabled services
    row = (
        lib.core.select(table="service", cols="SUM(enabled) as service_count")
        .exec(session)
        .first
    )
    result["service_count"] = (
        0 if row["service_count"] is None else int(row["service_count"])
    )
    ...
    with lib.core.MrsDbSession(session=session, check_version=False) as session:
        if lib.core.mrs_metadata_schema_exists(session):
            ...
            last_deployment_version = (
                schema_management.get_last_deployment_script_version(
            ...
            if (
                current_db_version[0] < lib.general.SUPPORTED_MAJOR_VERSION
                and not update_if_available
            ):
                raise Exception(
        ...
        if enable_mrs is not None:
            ...
            row = (
                lib.core.select(table="config", cols="service_enabled", where="id=1")
                .exec(session)
                .first
            )
...
def get_config_options(session):
    ...
    if row:
        row = (
            lib.core.select(table="config", cols=["data"], where="id=1")
            .exec(session)
            .first
        )
