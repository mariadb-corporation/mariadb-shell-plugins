...
arg_parser.add_argument(
    "--nocov", required=False, action="store_true", help="No coverage"
)
arg_parser.add_argument(
...
arg_parser.add_argument(
    ...
    required=False,
    action="store_true",
    help="Enable verbose mode for shell",
)
arg_parser.add_argument(
    "-u",
    "--userhome",
...
class MyPaths:
    def __init__(self, debug_mode, portable_path, shell_path, userhome_path: Path):
        ...
        self.runtime.plugins.mds_plugin = Path(
            os.path.join(self.runtime.plugins.root, "mds_plugin")
        )
        self.runtime.plugins.migration_plugin = Path(
            os.path.join(self.runtime.plugins.root, "migration_plugin")
        )
...
with pushd(paths.source.plugin):
    ...
    cov_options = ""
    init_path = os.path.join(paths.runtime.root.as_posix(), "init.d")
    enable_coverage_path = os.path.join(init_path, "launch_coverage.py")
...
mysqlsh.coverage = coverage.Coverage(source=[os.getcwd()])
mysqlsh.coverage.start()
"""

        # print("===== INIT SCRIPT =====")
        # print(init_script)
        # print("=======================")
        # cov_options = f"--cov={paths.source.code} --cov-append"
        os.mkdir(init_path)
        with open(enable_coverage_path, "w") as file:
            file.write(init_script)

    shell_args = ""
    if args.shell_verbose:
        shell_args = "--log-level=debug --verbose=2 --disable-builtin-plugins"

    OTHER_ARGS = []
    if other_arguments:
        OTHER_ARGS = other_arguments

    ARGS = ["--junitxml=plugin-tests.xml", "-W", "ignore::DeprecationWarning"]
...
exit_code = pytest.main([{(', ').join([f'"{arg}"' for arg in itertools.chain.from_iterable([LOGS, PATTERN, OTHER_ARGS, ARGS])])}])

if hasattr(mysqlsh, "coverage"):
    mysqlsh.coverage.stop();
    mysqlsh.coverage.save();
    mysqlsh.coverage.html_report(directory=\"plugin-test-coverage\")
    mysqlsh.coverage.xml_report(outfile=\"plugin-test-coverage.xml\")
...
"""
    ...
    script_path = os.path.join(paths.runtime.root / "execute.py")
    with open(script_path, "w") as f:
        f.write(script)

    command = f"{paths.shell} {shell_args} --py -f {script_path}"
    ...
    if not args.nocov:
        os.remove(enable_coverage_path)
        os.rmdir(init_path)

if not shell.returncode == 0 and not args.shell_verbose:
    print("----------------------------------------")
    print("MYSQLSH log")
    print("----------------------------------------")
