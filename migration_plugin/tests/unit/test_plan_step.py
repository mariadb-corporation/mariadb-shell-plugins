...
@pytest.fixture
def plan(temp_dir):
    context = migration.new_project(name="testproj", source_url="root@localhost")
...
def set_target(
    ...
    assert (
        root_id
    ), "MIGRATION_TEST_ROOT_COMPARTMENT_ID environment variable must be set"

    # either all 3 options are set or none
    assert (
        (public_subnet_id is not None)
        == (private_subnet_id is not None)
        == (vcn_id is not None)
    )
    ...
    if reuse_compartment:
        mysql_comp = oci_utils.Compartment.find_by_name(
            oci_config, parent_compartment_id=root_id, name="MySQL"
        )[0]
    else:
        mysql_comp = None
    ...
    if reuse_compute:
        ...
        compute_options = {"computeId": instance.id, "computeName": ""}
    else:
        compute_options = {"computeName": "test-jump-host"}
    ...
    if reuse_db:
        ...
        db_options = {"dbSystemId": instance.id, "name": ""}
    else:
        db_options = {"name": "test-mysql"}

    if vcn_id:
        net_options = {
            "vcnId": vcn_id,
            "publicSubnet": {"id": public_subnet_id},
            "privateSubnet": {"id": private_subnet_id},
        }
    else:
        net_options = {
...
def test_update_all():
    ...
    for s in steps:
        if s["id"] == model.SubStepId.PREVIEW_PLAN:
            ...
            assert s["status"] in ("IN_PROGRESS", "NOT_STARTED", "READY_TO_COMMIT"), s
...
""")
    ...
    # invalid profile
    update = plan_step.plan_update([{"id": my_id, "values": {"profile": "badprofile"}}])
    ...
    assert j(update) == [
        {
            ...
            "id": my_id,
            "status": "ERROR",
            "values": {"configFile": str(oci_conf_path), "profile": "badprofile"},
        }
    ]
    ...
    update = plan_step.plan_update([{"id": my_id, "values": {"profile": "DEFAULT"}}])
    assert j(update) == [
        {
            "data": None,
            "errors": [],
            ...
            "values": {"configFile": str(oci_conf_path), "profile": "DEFAULT"},
        },
    ]
...
def test_update_source(plan, sandbox_session):
    ...
    def normalize_access_denied(info, user):
        assert info["errors"][0]["message"].startswith("Access denied for user")
        info["errors"][0][
            "message"
        ] = f"Access denied for user '{user}'@'localhost' (using password: NO)"
    ...
    assert set(info["data"]["serverInfo"].keys()) == set(
        [
            ...
            "versionComment",
            "hasMRS",
            "numAccountsOnMysqlNativePassword",
            "numAccountsOnOldPassword",
        ]
...
def test_update_type_cold(plan, sandbox_session):
    ...
    assert j(update) == [
        {
            "data": {"allowedConnectivity": [], "allowedTypes": ["cold"]},
            "errors": [
                {
                    "info": None,
                    "type": None,
...
def test_update_type_hot_checks_localhost(plan, sandbox_repl_session):
    ...
    source_uri = "admin@localhost:" + sandbox_repl_session.uri.split(":")[-1]
    set_source(source_uri)
                ...
                "issues": [
                    {
                        ...
                        "level": "ERROR",
                        "objects": ["user:'root'@'%'", "user:'root'@'localhost'"],
                    ...
                    },
                    {
                        ...
                        "choices": ["strip_restricted_grants", "EXCLUDE_OBJECT"],
    ...
    assert j(update) == [
        {
            "data": {
                "issues": [
                    {
                        "check_id": "user/no_password",
                        "choices": ["lock_invalid_accounts", "EXCLUDE_OBJECT"],
                        "description": "The following user accounts have no password and will be locked in the target MySQL.\n\n        The listed accounts have a blank password, which is not allowed in the\n        MySQL HeatWave Service. They will still be migrated, but they will\n        be created LOCKED and their password must be changed before they can\n        connect to MySQL.\n\n        You may also set a password before migrating or exclude the user from\n        being migrated.\n        ",
                        "level": "NOTICE",
                        "objects": ["user:'root'@'%'", "user:'root'@'localhost'"],
...
def test_update_options(sandbox_session):
    ...
    configs = {
        "data": {
            "compartmentPath": "mysqltooling (root)/AlfredoHome/MySQL",
    ...
    update = plan_step.plan_update([{"id": my_id}])
    assert normeq([strip_sysvars(i) for i in j(update)][0], configs)
