...
class ProtocolBuilder:
    ...
    def __init__(
        ...
        self._results_pre = None
        self._results_mapping = []
        self._results_post = None
    ...
    def build(self) -> None:
        ...
        if handle_type_hints:
            for param in self._definition.parameters:
                ...
                if info["name"] not in hints:
                    handle_type_hints = False
                    break
    ...
    def _generate_protocol_bindings(self, bindings: dict) -> str:
    ...
    def _generate_parameter_bindings(self, bindings: dict, nullables: dict) -> list:
    ...
    def _generate_return_value_bindings(self, bindings: dict) -> None:
        ...
        for func_name in bindings:
            ...
            self._results_mapping.append(line)
        if errors:
            raise Exception(f"Errors processing results: {errors}")

    def _process_results_file(self):
        results_file = self._load_file_content("results_")
        result_lines = results_file.split("\n")

        index = 0
        mapping_interface = f"export interface IProtocol{self._plugin_name}Results"
        while index < len(result_lines):
            if self._results_pre is None:
        ...
        # The case when no results are hardcoded anymore
        if len(self._results_mapping) == 0:
            self._results_pre = result_lines

    def _generate_results(self):
        results = ""
        if self._results_pre is not None:
            results += "\n".join(self._results_pre)

        results += f"\nexport interface IProtocol{self._plugin_name}Results {{\n"
        results += "\n".join(self._results_mapping)
        results += "\n}\n"

        if self._results_post is not None:
            results += "\n".join(self._results_post)
...
class TypeScriptFunction:
    ...
    def __init__(
        ...
        if use_type_hints:
            self._type_hints = get_type_hints(self._function)
    ...
    def get_type_hint_interfaces(self):
    ...
    def handle_return_value(self) -> None:
        ...
        if self._type_hints is None:
            # This function only works when use of type hints is enabled for the function.
            return
