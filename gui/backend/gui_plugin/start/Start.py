...
def web_server(
    ...
    secure=None,
    webrootpath=None,
    single_instance_token=None,
    read_token_on_stdin=False,
    ...
    if (
        single_instance_token is not None or read_token_on_stdin
    ) and single_server is not None:
        raise ValueError(
    ...
    try:
            ...
            if type(secure) is dict:
                if secure.get("tempCerts", False):
                    ...
                    if "keyfile" not in secure or secure["keyfile"] == "default":
                        ...
                        secure["keyfile"] = path.join(*[cert_path, "server.key"])
                    if "certfile" not in secure or secure["certfile"] == "default":
                        default_cert_used = True
                        secure["certfile"] = path.join(*[cert_path, "server.crt"])
            else:
                raise ValueError(
                    "If specified, the secure parameter need to " "be of type dict"
                )
            ...
            if not accept_remote_connections:
                ...
                if default_cert_used:
                    try:
                        ...
                        logger.info(
                            "\tCertificate is not correctly installed. "
        ...
        server = ThreadedHTTPServer(
            ("127.0.0.1" if not accept_remote_connections else "0.0.0.0", port),
                ...
                logger.add_filter(
                    {
                        ...
                        "end": " HTTP",
                        "expire": (
                            Filtering.FilterExpire.OnUse
                            if SystemUtils.in_vs_code()
                            else Filtering.FilterExpire.Never
                        ),
                    }
                )
            # Start web server
...
def native_ui():
    ...
    wrappers_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "wrappers")
    )
...
def browser_app():
    ...
    p_web_server = Popen(
        [
            ...
            "--py",
            "-e",
            f'gui.start.web_server(port={port}, secure={certs}, single_instance_token="{token}")',
        ]
    )
