...
class BackendDatabase:
    ...
    def __enter__(self):
        if self.db is None:
            self.db = GuiBackendDb.get_instance(log_rotation=self.log_rotation)
...
class GuiBackendDb:
    ...
    def __init__(self, log_rotation=False, session_uuid=None):
        ...
        backend_db_manager = BackendSqliteDbManager(
            log_rotation=log_rotation, session_uuid=self._session_uuid
        )
    ...
    def get_instance(cls, log_rotation=False, session_uuid=None):
        ...
        with _backend_db_lock:
            if key not in _backend_db_instances:
                ...
                _backend_db_instances[key] = cls(
                    log_rotation=log_rotation, session_uuid=session_uuid
                )
            else:
        ...
        with _backend_db_lock:
            for key, instance in list(_backend_db_instances.items()):
                try:
                    ...
                    logger.error(
                        f"Error closing GuiBackendDb instance for key {key}: {e}"
                    )
                finally:
    ...
    def get_connection_details(self, db_connection_id):
        ...
        """
        res = self.execute(
            """SELECT db_type, options, settings
                                FROM db_connection
                                WHERE id = ?""",
            (db_connection_id,),
        ).fetch_one()
        if not res:
            raise Error.MSGException(
                Error.DB_INVALID_CONNECTION_ID,
    ...
    def log(self, event_type, message):
        ...
        self._db.execute(
            """INSERT INTO `gui_log`.`log`(event_time, event_type, message) VALUES(?, ?, ?)""",
            (datetime.datetime.now(), event_type, message),
        )

    def message(self, session_id, is_response, message, request_id):
        self._db.execute(
            """INSERT INTO `gui_log`.`message`(session_id, request_id, is_response,
            message, sent) VALUES(?, ?, ?, ?, ?)""",
...
def convert_workbench_sql_to_sqlite(sql):
    ...
    sql = re.sub(
        r"(^SET\s\@OLD_FOREIGN_KEY_CHECKS.*\=0;$)",
        "PRAGMA foreign_keys = OFF;",
        sql,
        flags=re.MULTILINE,
    )
    sql = re.sub(
        r"(^SET\sFOREIGN_KEY_CHECKS.*$)",
...
def convert_workbench_sql_file_to_sqlite(mysql_sql_file_path):
    ...
    try:
            ...
            if add_constraints:
                ...
                if prev_version_sql == "":
                    ...
                    filename_match = re.match(
                        r"(.*?)(\d+\.\d+\.\d+)_to_(\d+\.\d+\.\d+)((.|\.)*)", file_name
                    )
                    if not filename_match:
                        # cspell:ignore myschema
                        raise Exception(
                            f"The file contains an ALTER TABLE "
                    ...
                    previous_version_file_path = path.join(
                        path.dirname(mysql_sql_file_path), g[0] + g[1] + g[3]
                    )
                ...
                original_create_table_m = re.findall(
                    r"(CREATE\s+TABLE.*" + re.escape(table_name) + r"\s*\((.|\n)*?;)",
                    prev_version_sql,
                    flags=re.MULTILINE,
                )
                if not original_create_table_m:
                    raise Exception(
                        f"CREATE TABLE {table_name} statement not "
                ...
                columns = re.findall(
                    r"^\s*(`[\w_\d]*`)", original_create_table, flags=re.MULTILINE
                )
                ...
                sql = re.sub(
                    re.escape(old_alter_table_stmt),
                    replacement_script,
                    sql,
                    flags=re.MULTILINE,
                )
        ...
        create_tables = re.findall(
            r"(CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s*(`\w*`" r")\s*\(\s*((.|\n)*?);)",
            sql,
            flags=re.MULTILINE,
        )
        for g in create_tables:
            ...
            # INDEX `fk_user_group_has_user_user1` (`user_id` ASC),
            actions_m = re.findall(
                r"((UNIQUE\s+)*INDEX\s+([`\w_\d]*)\s+\s*(\(" r"[`\w_\d\s,]+\))\s*,\s*)",
                actions,
                flags=re.MULTILINE,
            )
            if actions_m:
                for g in actions_m:
                    ...
                    # remove
                    new_create_table_stmt = re.sub(
                        re.escape(index_line),
                        "",
            ...
            if alter_index_stmt != "":
                sql = re.sub(
                    re.escape(old_create_table_stmt),
                    f"{new_create_table_stmt}\n\n{alter_index_stmt}\n",
                    sql,
                    flags=re.MULTILINE,
                )

        sqlite_sql_file_path = re.sub(r"(\.mysql\.)", ".sqlite.", mysql_sql_file_path)
...
def convert_all_workbench_sql_files_to_sqlite(directory=None):
    ...
    """
    if not directory:
        directory = path.join(path.dirname(path.abspath(__file__)), "db_schema")

    # loop over all *.mysql.sql files in ./db_schema
    for f_name in listdir(directory):
        if path.isfile(path.join(directory, f_name)) and f_name.endswith(".mysql.sql"):
