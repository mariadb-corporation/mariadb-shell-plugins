...
def test_get_catalog_object_names_by_shell(shell_session):
    ...
    assert all(
        schema in schemas for schema in ["information_schema", "performance_schema"]
    )

    result = mysqlsh.globals.gui.db.get_catalog_object_names(
        shell_session, "Character Set"
    )
...
def test_get_catalog_object_by_shell(shell_session):
    result = mysqlsh.globals.gui.db.get_catalog_object(
        shell_session, "Schema", "information_schema"
    )
    assert "name" in result
    assert result["name"] == "information_schema"

    result = mysqlsh.globals.gui.db.get_catalog_object(
        shell_session, "Character Set", "ascii"
    )
    assert "name" in result
...
def test_get_schema_object_names_by_shell(shell_session):
    result = mysqlsh.globals.gui.db.get_schema_object_names(
        shell_session, "Table", "mysql"
    )
...
def test_get_schema_object_by_shell(shell_session):
    result = mysqlsh.globals.gui.db.get_schema_object(
        shell_session, "Table", "mysql", "user"
    )
    assert "name" in result
...
def test_get_table_object_names_by_shell(shell_session):
    result = mysqlsh.globals.gui.db.get_table_object_names(
        shell_session, "Index", "mysql", "user"
    )
...
def test_get_table_object_by_shell(shell_session):
    result = mysqlsh.globals.gui.db.get_table_object(
        shell_session, "Index", "mysql", "user", "PRIMARY"
    )
    assert "name" in result
