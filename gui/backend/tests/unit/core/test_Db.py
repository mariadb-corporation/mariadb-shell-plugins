...
import shutil
from gui_plugin.core.Db import (
    GuiBackendDb,
    BackendSqliteDbManager,
    convert_workbench_sql_file_to_sqlite,
...
def test_GuiBackendDb_commit():
    ...
    backend_db.execute(
        """INSERT INTO log(event_time, event_type,
            message) VALUES(?, ?, ?)""",
...
def test_GuiBackendDb_check_for_previous_version_and_upgrade():
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        backend_db = BackendSqliteDbManager(
            log_rotation=False, session_uuid=None, connection_options=connection_options
        )
...
def test_upgrade_db():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        conn = sqlite3.connect(
            os.path.join(tmpdirname, "mysqlsh_gui_backend_0.0.11.sqlite3")
        )
        cur = conn.cursor()
...
def test_backup_logs():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        assert os.path.exists(
            os.path.join(tmpdirname, "mysqlsh_gui_backend_log.sqlite3")
        )
        connection_options = {
            "db_dir": tmpdirname,
            "database_name": "main",
            ...
            "attach": [
                {
                    ...
                    "db_file": os.path.join(
                        tmpdirname, f"mysqlsh_gui_backend_log.sqlite3"
                    ),
                }
            ],
        }
        db_manager = BackendSqliteDbManager(
            log_rotation=False, session_uuid=None, connection_options=connection_options
        )
