...
def sqlite_session():  # pragma: no cover
    ...
    db_path = os.path.abspath(
        os.path.join(
            mysqlsh.plugin_manager.general.get_shell_user_dir(),  # pylint: disable=no-member
            "plugin_data",
            "gui_plugin",
            f"user_3",
            "tests.sqlite3",
    ...
    db_session = db_session_class(uuid.uuid1(), True, {"db_file": "tests.sqlite3"})
...
def mysql_connections_exists():  # pragma: no cover
    ...
    result = DbConnections.add_db_connection(
        1,
        {
            "db_type": "MySQL",
    ...
    results.append({"id": connection_id, "connection": connection})
...
def mysql_sessions(mysql_connections_exists):  # pragma: no cover
    ...
    try:
        ...
        for index in range(10):
            json_object = {"key1": f"key1_value_{index}", "key2": f"key2_value_{index}"}

            db_session.execute(
                f"""INSERT INTO tests.test1 (column1, column2, column3, last_update)
                VALUES
                ('column1_{index}',
                'column2_{index}',
...
class TestDbSessionSqlite:
    ...
    def test_instance(self):
        ...
        assert (
            str(exp.value)
            == "Error[MSG-1202]: The 'db_file' option was not set for the '' database."
        )
        ...
        db_session = DbSqliteSession(uuid.uuid1(), False, {"db_file": "tests.sqlite3"})
        assert db_session is not None
        db_session.close()

        assert os.path.isfile("tests.sqlite3")
        assert oct(os.stat("tests.sqlite3").st_mode) == oct(
    ...
    def test_rollback(self, sqlite_session):
        ...
        sqlite_session.execute(
            """INSERT INTO log(event_time, event_type, message)
            VALUES (?, ?, ?)""",
        ...
        rows = sqlite_session.execute(
            "SELECT * FROM log WHERE event_time=?",
            (now,),
    ...
    def test_select(self, sqlite_session):
        ...
        sqlite_session.execute(
            "SELECT * FROM log",
            request_id=validate1.request_id,
            callback=validate1,
            options=validate1.options,
        )
        ...
        for index in range(5):
            ...
            sqlite_session.execute(
                """INSERT INTO log(event_time, event_type, message)
                VALUES (?, ?, ?)""",
                (now, "error", f"just some message {index}"),
                callback=insert_callback,
                request_id=insert_callback.request_id,
            )
        insert_callback.join_and_validate()
        ...
        sqlite_session.execute(
            "SELECT * FROM log",
            request_id=validate2.request_id,
            callback=validate2,
            options=validate2.options,
        )
...
class TestDbMysqlSession:
    ...
    def test_select(self, mysql_sessions):
        ...
        def func(state, message, request_id, values):
            ...
            if (
                "result_format" not in func.options
                or func.options["result_format"] == "list"
            ):
                assert values["columns"] == [
                    {"length": 5, "name": "id", "type": "SMALLINT"},
        ...
        session.execute(
            "SELECT * FROM tests.test1 where column1 like ? AND column2 like ?",
            request_id=func.request_id,
    ...
    def test_start_transaction_commit(self, mysql_sessions):
        ...
        def count_callback(state, message, request_id, values):
            ...
            assert {"length": 21, "name": "cnt", "type": "INT"} in values["columns"]
        ...
        session.execute(
            f"""SELECT COUNT(*) as cnt FROM tests.test1""",
