...
test.describe("Aborted Migration", () => {

    // eslint-disable-next-line no-empty-pattern
    test.beforeAll(async ({ }, testInfo) => {
        await Misc.loadPage(testInfo.titlePath[1], constants.MockMigrationStatusEnum.Aborted);
    ...
    test("Aborted Migration", async () => {
        ...
        // Abort Migration
        await migrationAssistant.abortMigration();
        ...
        for (const tile of tiles) {
            ...
            for (const step of tile.subSteps!) {
                if (constants.migrationPlanSubSteps.includes(step.name!)) {
                    ...
                    expect(step.status, `Expected ${step.name}`).toBe(constants.TileStepStatusEnum.Running);
        ...
        for (const step of provisioningSteps!) {
            ...
            expect(step.status).toBe(constants.StepStatusEnum.Aborted);
            expect(step.description).toContain("Operation aborted");
        }
