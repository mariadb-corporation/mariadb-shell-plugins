...
export class Misc {
     ...
     */
    public static untilNotificationExists = (text: RegExp): Condition<boolean | undefined> => {
        return new Condition(`for notification '${text.toString()}' to exist`, async () => {
            const notifications = await this.getToastNotifications();

            for (const notification of notifications) {
                if (notification?.message.match(text) !== null) {
                    return true;
                }
            }
        });
